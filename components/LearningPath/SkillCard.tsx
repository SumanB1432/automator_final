...
const SkillCard: React.FC<SkillCardProps> = ({ skill, isLocked = false }) => {
  ...
  return (
    <>
      ...
      <Card className={`skill-card bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)] ${skill.isCompleted ? 'border-[#0FAE96]' : ''} ${isLocked ? 'opacity-60' : ''}`}>
        <CardHeader className="p-4 pb-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <Badge className={`text-xs font-inter ${skill.isCompleted ? 'border-[rgba(255,255,255,0.05)] text-[#B6B6B6]' : 'bg-[#0FAE96] text-white'}`}>
              ...
              >
                ...
                <span className="sr-only">{expanded ? 'Collapse' : 'Expand'} skill details</span>
              </Button>
            </div>
          </div>
            ...
            <div className="space-y-3 mt-3">
              {skill.videos.length === 0 ? (
                ...
                skill.videos.map((video) => (
                  <div
                    key={video.id}
                    className={`rounded-md border border-[rgba(255,255,255,0.05)] ${video.isCompleted ? 'bg-[rgba(15,174,150,0.05)] border-[rgba(15,174,150,0.2)]' : 'bg-[rgba(255,255,255,0.02)]'}`}
                  >
                    <div className="flex items-start p-3">
                      ...
                      <Button
                        className={`ml-2 h-10 w-10 ${video.isCompleted ? 'border-[rgba(255,255,255,0.05)] text-[#B6B6B6]' : 'bg-[#0FAE96] text-white'} focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#0FAE96] transition-colors`}
                        onClick={() => handleVideoClick(video)}
                      ...
                      >
                        ...
                        <span className="sr-only">{video.isCompleted ? 'Video completed' : 'Play video'}</span>
                      </Button>
                    </div>
                  </div>
                ))
