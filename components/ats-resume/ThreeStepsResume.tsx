...
export default function ThreeStepsResume() {
  ...
  const steps = [
    {
      ...
      description: "Find what’s missing and create the perfect ATS resume.",
      icon: "/images/resume.svg",
    },
    {
      id: 2,
      title: "Learn & Apply with AI",
      description: "Fill skill gaps for free and auto-apply to jobs.",
      icon: "/images/skill.svg",
    },
    {
      id: 3,
      title: "Get Hired",
      description: "Stand out. Get interviews. Land your dream job.",
      icon: "/images/job.svg",
    },
  ];
  ...
  useEffect(() => {
    ...
    stepRefs.current
      ...
      .forEach((step) => observer.observe(step as Element));

    return () => {
      stepRefs.current
        .filter((step) => step !== null)
        .forEach((step) => observer.unobserve(step as Element));
    };
  }, []);
    ...
    <div className="text-white py-16 px-6 lg:px-24">
      <div className="max-w-4xl mx-auto flex flex-col items-center space-y-6">
        ...
        <p className="text-gray-400">
          Build your resume in one call, fix skill gaps, auto-apply with AI, and get hired for your dream job—faster than ever.
        </p>
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="bg-[#2C223B] w-12 h-12 flex items-center justify-center rounded-full">
                <img src={step.icon} alt={`Icon for ${step.title}`} className="w-6 h-6" />
              </div>
